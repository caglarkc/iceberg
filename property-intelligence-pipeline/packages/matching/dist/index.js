import { addressMatchScore, appointmentProximityScore, classifyQueue, contactNameMatchScore, minutesBetween, signalChips, titleMatchScore, userMatchScore, weightedConfidence } from "./scoring.js";
export function matchRecording(input) {
    ...
    const candidates = companyProperties.map((property) => {
        ...
        const appointment = input.appointments
            ...
            .sort((a, b) => Math.abs(minutesBetween(recordedAt, new Date(a.scheduled_at))) -
            Math.abs(minutesBetween(recordedAt, new Date(b.scheduled_at))))[0];
