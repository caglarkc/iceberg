...
export function extractUkPostcodes(text: string): string[] {
  const matches = text.match(UK_POSTCODE) ?? [];
  return [...new Set(matches.map((p) => normalizePostcode(p)))];
}

export function extractStreetAddresses(text: string): Array<{ number?: string; street: string }> {
  const results: Array<{ number?: string; street: string }> = [];
...
export function jaroWinkler(a: string, b: string): number {
  ...
  const aMatches = new Array<boolean>(aLower.length).fill(false);
  const bMatches = new Array<boolean>(bLower.length).fill(false);
  ...
  for (let i = 0; i < aLower.length; i += 1) {
    ...
    if (aLower[i] !== bLower[k]) t += 1;
    k += 1;
  }

  const m = matches;
  const jaro =
    (m / aLower.length + m / bLower.length + (m - t / 2) / m) / 3;

  let prefix = 0;
  for (let i = 0; i < Math.min(4, aLower.length, bLower.length); i += 1) {
    if (aLower[i] === bLower[i]) prefix += 1;
    else break;
  }
