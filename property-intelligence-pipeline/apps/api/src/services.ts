...
export class IngestService {
  constructor(
    private readonly store: PipStore,
    private readonly plaud: PlaudProviderAdapter,
    private readonly config: AppConfig
  ) {}

  async ingestFromMock(): Promise<PlaudRecording[]> {
    const rawList = await this.plaud.listRecordings();
    return rawList.map((raw) => this.persistRaw(raw));
  }

  async ingestRecording(providerId: string): Promise<PlaudRecording> {
  ...
  private persistRaw(raw: PlaudRecordingRaw): PlaudRecording {
...
export class MatchService {
  constructor(private readonly store: PipStore, private readonly config: AppConfig) {}
  ...
  confirmMatch(recordingId: string, propertyId: string, actorId: string): TranscriptMatch {
    ...
    if (!match) throw new Error("Match not found — run match first");
    const candidate = match.candidates_json.find((c) => c.property_id === propertyId);
    if (!candidate) throw new Error("Property not in candidate list");
...
export class ExtractService {
  ...
  async extract(recordingId: string) {
    ...
    for (const key of Object.keys(rawFields) as Array<keyof PropertyProposalExtraction["fields"]>) {
      penalized[key] = applyEvidencePenalty(rawFields[key], transcript.text) as never;
    }
    const fields = penalized as PropertyProposalExtraction["fields"];
...
export class ReviewService {
  constructor(private readonly store: PipStore) {}
  ...
  decide(
    ...
    for (const [key, status] of Object.entries(decisions)) {
      const fieldKey = key as keyof PropertyProposalExtraction["fields"];
      if (nextFields[fieldKey]) {
        nextFields[fieldKey] = { ...nextFields[fieldKey], status } as never;
      }
    }
    const fields = nextFields as PropertyProposalExtraction["fields"];
    ...
    const review_status =
      approved > 0 && rejected === 0 ? "approved" : approved > 0 ? "partial" : "rejected";
...
export class ApplyService {
  ...
  apply(recordingId: string, actorId: string) {
    ...
    for (const [key, field] of Object.entries(extraction.fields_json)) {
      if (field.status !== "approved" || field.value === null) continue;
